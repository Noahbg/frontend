<template>
    <modal has-alerts="server.schedules.manage" :title="`server.schedules.${task ? 'edit_task' : 'create_task'}`"
        ...
        <v-form :service-id="submit">
            <div class="flex flex-col lg:flex-row">
                <v-select class="lg:w-1/2 lg:mr-4" name="action" prefix="server.schedules.actions" :options="allowedActions"
                    v-model:value="action" rule="required" />

                <v-input class="lg:w-1/2" name="time_offset" type="number" rule="required" :value="task?.timeOffset ?? 0" />
            </div>

            <v-select v-if="action === 'power'" name="payload" prefix="server.schedules.power_actions"
                :options="allowedPowerActions" :value="task?.payload ?? 'start'" rule="required" />

            <v-input v-if="action === 'command'" name="payload" :value="task?.payload" rule="required" />
...
export default defineComponent({
    ...
    setup(props) {
        ...
        onModelLoaded('server', server => {
            ...
            if (server.nest?.shortName === 'rust') {
                allowedActions.value.push('full_wipe', 'map_wipe');
            }
        });
